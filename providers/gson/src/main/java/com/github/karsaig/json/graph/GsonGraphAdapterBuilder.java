...
import com.github.karsaig.json.GsonDelegateJsonBuilder;
...
public final class GsonGraphAdapterBuilder implements GraphAdapterBuilder {
    ...
    @Override
    public GraphAdapterBuilder registerOn(JsonBuilder jsonBuilder) {
        GsonDelegateJsonBuilder builder = GsonDelegateJsonBuilder.class.cast(jsonBuilder);

        Factory factory = new Factory(instanceCreators);
        builder.registerTypeAdapterFactory(factory);
        for (Map.Entry<Type, InstanceCreator<?>> entry : instanceCreators.entrySet()) {
            builder.registerTypeAdapter(entry.getKey(), factory);
        }
        return this;
    }

    private GraphAdapterBuilder addType(Type type, InstanceCreator<?> instanceCreator) {
    ...
    static class Factory implements TypeAdapterFactory, InstanceCreator {
         ...
         * <p/>
         * <p>
         * Json should only ever call this method when we're expecting it to; that is only when we've called back into
         * Json to deserialize a tree.
         */
        @SuppressWarnings("unchecked")
        public Object createInstance(Type type) {
            Graph graph = graphThreadLocal.get();
